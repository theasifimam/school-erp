...
export default function ModernSchoolCalendar() {
  ...
  const [filters, setFilters] = useState<Filters>({
    types: Object.keys(EVENT_TYPES) as (keyof typeof EVENT_TYPES)[],
  ...
  const getFilteredEvents = (
    ...
    if (filterSettings.types.length > 0) {
      filtered = filtered.filter((e) =>
        filterSettings.types.includes(e.type as keyof typeof EVENT_TYPES)
      );
    }
  ...
  const handleApproveLeave = (eventId: number) => {
    setEvents(
      events.map((e) =>
  ...
  const handleRejectLeave = (eventId: number) => {
    setEvents(
      events.map((e) =>
                      ...
                      <Badge
                        ...
                        variant={
                          filters.types.includes(
                            type as keyof typeof EVENT_TYPES
                          )
                            ? "default"
                            : "outline"
                        }
                        ...
                        style={{
                          backgroundColor: filters.types.includes(
                            type as keyof typeof EVENT_TYPES
                          )
                            ? details.color
                            : "transparent",
                          color: filters.types.includes(
                            type as keyof typeof EVENT_TYPES
                          )
                            ? "white"
                            : details.color,
